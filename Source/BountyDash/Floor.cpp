...
AFloor::AFloor()
{
	...
	ConstructorHelpers::FObjectFinder<UStaticMesh>myMesh(TEXT("/Game/Destroyable_Mesh.Destroyable_Mesh"));
	ConstructorHelpers::FObjectFinder<UMaterialInterface>myMaterial(TEXT("/Game/StarterContent/Materials/M_Concrete_Tiles.M_Concrete_Tiles"));
...
void AFloor::Tick(float DeltaTime)
{
	...
	for (auto Scene : FloorMeshScenes)
	{
		...
		if (Scene->GetComponentTransform().GetLocation().X <= KillPoint)
		{
			...
			if (thisPiece)
			{
				thisPiece->Destructable->ApplyDamage(10000, thisPiece->GetActorLocation(), FVector(-FMath::RandRange(-10, 10), -FMath::RandRange(-10, 10), -FMath::RandRange(-10, 10)), 10000);
			}
